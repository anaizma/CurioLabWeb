...
// The REAL seeding chain, ending at a PENDING student ready to activate.
// EnrollmentService.create (seeding) now creates the inert student SHELL, the
// PENDING student membership, and the two form-sourced consents in ONE
// transaction — nothing is synthesized by hand here.
async function seededPendingStudent() {
  ...
  // Coupling D (seeding): the enrollment record + the inert student SHELL + the
  // PENDING student membership + the two form-sourced consents, all in one real
  // transaction. Nothing is synthesized by hand.
  ...
  return {
    ...
    director,
    accountId: result.studentAccountId!,
    enrollmentRecordId: result.enrollmentRecordId,
    membershipId: result.studentMembershipId!,
  }
}
