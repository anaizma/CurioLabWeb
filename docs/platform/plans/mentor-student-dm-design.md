...
1. **Model and provisioning:** the column-encryption-at-rest seam (C.11) first; then the `safety_officer` role + assignment (with the not-a-peer enforcement), the `mentor_dm` signed-form consent grant type, the `dm_thread`/`dm_message` append-only encrypted tables with the four-party access model and the visibility header, and the `canDirectMessage` predicate. Guard amendment (behind counsel note) + the enable-precondition gate. Off by default. **— BUILT and DARK (2026-07-24, `feat/platform-m1`).** Fully implemented against synthetic data, `MENTOR_DM_ENABLED=false`, COUNSEL-GATED: nothing runs against a real minor while the flag is off. Enabling requires the Part A board ratification + the Part B counsel sign-off (the COPPA posture change and the no-direct-messaging guard amendment) + every Part D enable-precondition satisfied in the system. See `BUILD-STATUS.md` and `api-reference.md` §10.
2. **Structural constraints:** hours, off-platform contact-info flagging, export, and the retention carve-out in deletion-fulfill. No attachments. **— BUILT and DARK (2026-07-24, `feat/platform-m1`).** Migration `0031_mentor_student_dm_phase2.sql` (per-chapter closed-hours override on `chapter`; the append-only `dm_flag` content-flag table). Closed hours enforced on SEND only, computed from the chapter timezone at an injected `now` (default 07:00–21:00, per-chapter overridable; reads never gated; `DmClosedHoursError`). A pure, data-driven contact-info detector (`detectDmContentFlags`, `@curiolab/core`) writes an append-only `dm_flag` on send (does not block) and backs a pre-send check (`POST /api/dm/check-draft`, inert). Full-thread export (`GET /api/dm/threads/[id]/export`) scoped to the student or a verified guardian, dark-gated. The retention carve-out excludes `dm_thread`/`dm_message` from deletion-fulfill (config `DM_RETENTION_MS`, a placeholder pending counsel ~age 30); disclosed at consent (C.10). Synthetic data only, `MENTOR_DM_ENABLED=false`. See `BUILD-STATUS.md` and `api-reference.md` §10.
3. **Detection and oversight:** content-flag matchers + `dm_flag` + routing; the full-coverage chronological reading queue with pinned flags and per-thread read-receipts; the monitoring-ledger extension and the quarterly officer-oversight export; the guarded two-adult guardian-visibility suspension with the 90-day expiry and the reporter checkpoint; mentor-departure freeze/handoff. Behavioral signals are NOT built (C.6): documented as a scale-triggered future capability with a defined transition threshold. **— BUILT and DARK (2026-07-24, `feat/platform-m1`).** Migration `0032_mentor_student_dm_phase3.sql` (append-only `dm_read_receipt`, `dm_flag_review`, event-sourced `dm_visibility_suspension`, `dm_thread_freeze`). The four remaining C.5 categories added to `DM_CONTENT_MATCHERS` (secrecy_framing, in_person_arrangement, romantic_appearance, home_life_probing); every matched category flags on send + appends `dm.flag_raised`. `DmOversightService` = the full-coverage decrypted reading queue (flags pinned, per-thread read/unread coverage, the `DM_FULL_REVIEW_MAX_WEEKLY_MESSAGES` transition threshold, and the separate guardian-never-opened supervision signal), read-receipts (`dm_read_receipt` → 100% computed coverage), flag disposition, and the quarterly `dmOversightReport`. The monitoring ledger extends `writeAccessLedger` (officer/guardian reads, flag raise/review, receipt, the suspension lifecycle, freeze) and has no update/delete path (append-only floor). `DmVisibilitySuspensionService` = the two-adult, reason-required, 90-day-expiry, reporter-checkpoint suspension; an active one removes the guardian from the party-check; a mentor cannot be the second adult; auto-restores on expiry. `runDmFreezeOnDeparture` freezes/preserves a departed mentor's threads (rejects sends, still reads, never deletes, records the handoff decision). New caps `dm.suspend_guardian_visibility` + `dm.acknowledge_visibility_suspension` (staff-only, no student party, exempt from the no-DM guard). Synthetic data only, `MENTOR_DM_ENABLED=false`. See `BUILD-STATUS.md` and `api-reference.md` §10.
