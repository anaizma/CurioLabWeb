...
**Deferred to Phases 3–4:** the safety-officer reading queue + read-receipts + the other content-flag categories (secrecy framing, in-person arrangements, romantic/appearance, home-life probing), the monitoring-ledger extension + quarterly export, the two-adult guardian-visibility suspension, mentor-departure freeze (Phase 3); the participant send/read + first-open onboarding + report endpoints, the safety-officer queue endpoints, guardian read/digest (Phase 4). Attachments are OUT (v1 text-only). The participant HTTP send/read surface is not built (the send SERVICE, with hours + contact-info flagging, exists and is tested).
