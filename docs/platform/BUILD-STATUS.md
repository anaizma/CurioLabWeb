...
### Phase 2 — structural constraints (built DARK, this pass)

Built exactly as design Part E.2, all behind `MENTOR_DM_ENABLED` (default off), synthetic data + deterministic clocks. Migration `0031_mentor_student_dm_phase2.sql` (proved RED@0030 → GREEN@0031); Drizzle mirror updated.

- **Closed hours (C.4):** a DM **send** is refused outside the chapter's allowed **local** window (default 07:00–21:00, `[open, close)`), computed deterministically from the chapter `timezone` at an injected `now` (`DmThreadService.assertWithinAllowedHours`). Per-chapter override columns `chapter.dm_open_hour` / `dm_close_hour` (0031; NULL = config default `dmOpenHourDefault`/`dmCloseHourDefault` = 7/21). Outside → `DmClosedHoursError` (409), writes nothing. **Reads are never hours-gated.**
- **Off-platform contact-info flagging (C.4/C.5):** a **pure, data-driven** detector `detectDmContentFlags` (`@curiolab/core`) — phone / email / social handle / off-platform URL, category `contact_info`; extensible for Phase 3 by adding matcher rows. On a send, a match writes an append-only `dm_flag` (`{id, thread_id, message_id, category, detail, created_at}`, 0031; `detail` = the matched KIND, never raw plaintext) routed to the safety officer — it does **not** block the send. Detection runs on the **plaintext** at send time, never on ciphertext. Pre-send check exposed as `DmThreadService.checkDraft` + `POST /api/dm/check-draft` (session-gated, writes nothing, **inert** in the manifest) so the frontend interstitial can check a draft without sending.
- **Thread export (C.4):** `DmThreadService.exportThread` + `GET /api/dm/threads/[threadId]/export` — the full **decrypted** thread in `seq` order, scoped so **only** the student (own thread) or a **verified guardian** of that student can export; anyone else (mentor, safety officer, stranger, other chapter) gets an opaque 403. GET (read-exempt from the manifest); dark-gated (flag off → refuses).
- **Retention carve-out (C.11):** `DeletionFulfillmentService.fulfillDeletion` **excludes** `dm_thread`/`dm_message` from deletion-request fulfillment — the fulfill path never enumerates them, so a subject's DM logs are retained even when the rest of their record is erased/anonymized; the fulfillment audit records `dmThreadsRetained` so the exclusion is explicit. Retention interval config `DM_RETENTION_MS` (a **placeholder** pending counsel; ~age 30). Disclosed at consent (C.10).

**No new capabilities** and **no guard change** were needed for Phase 2: hours + contact-info flagging ride the existing `dm.message` send service; the pre-send check is a pure detector (inert POST); export does direct student-or-guardian scoping (no registry capability, GET read-exempt). The no-direct-messaging guard still returns `[]` for the real REGISTRY.

### Phase 3 — detection & oversight (built DARK, this pass)

Built exactly as design Part E.3, all behind `MENTOR_DM_ENABLED` (default off), synthetic data + deterministic clocks. Migration `0032_mentor_student_dm_phase3.sql` (proved RED@0031 → GREEN@0032); Drizzle mirror updated. All four new tables are **append-only** (shared `reject_append_only_mutation` trigger + SELECT/INSERT-only grants), like the Phase 1/2 store.

- **Content-flag categories (C.5):** `DM_CONTENT_MATCHERS` (`@curiolab/core`) extended with `secrecy_framing`, `in_person_arrangement`, `romantic_appearance`, `home_life_probing` (data-driven rows; `detail` = matched KIND). On a send **every** matched category writes one `dm_flag` **and** appends a `dm.flag_raised` monitoring-ledger entry; ordinary mentoring text flags nothing. Never auto-blocks.
- **Full-coverage reading queue (C.6):** `DmOversightService.readingQueue` + `GET /api/ops/dm/queue` (`dm.oversee`, GET read-exempt) — the **complete chronological, decrypted** queue for the officer's chapter, flags **pinned**, per-thread read/unread coverage, `overFullReviewThreshold` from `DM_FULL_REVIEW_MAX_WEEKLY_MESSAGES` (placeholder), and the separate `guardianEverOpened` supervision signal (drives a guardian nudge; **not** a mentor risk score — there is none). NOT a ranked sample (deferred, scale-triggered). Logs one `dm.read_by_officer` per thread. A non-officer / other-chapter officer → opaque 403.
- **Read-receipts + coverage (C.6):** `dm_read_receipt` (0032) + `DmOversightService.markThreadRead` + `POST /api/ops/dm/threads/[threadId]/read` (`dm.oversee`) — records a receipt to the latest `seq`; coverage is **computed** (`seq ≤ max(up_to_seq)`), so a mark-read = 100% coverage. `dm.receipt_recorded` logged.
- **Monitoring ledger + officer oversight (C.7):** `writeAccessLedger` extended (`dm.read_by_officer`, `dm.read_by_guardian`, `dm.flag_raised`, `dm.flag_reviewed`, `dm.receipt_recorded`, `dm.visibility_{suspended,acknowledged,revoked,restored}`, `dm.thread_frozen`). Officer/guardian reads log (guardian reads via `DmThreadService.readThread`). No update/delete path (append-only floor — the officer cannot modify it). Flag disposition: `dm_flag_review` (0032) + `DmOversightService.reviewFlag` + `POST /api/ops/dm/flags/[flagId]/review`. Quarterly board export `dmOversightReport({sql,chapterId,from,to})` + `GET /api/ops/dm/oversight-report` (`dm.oversee`, GET read-exempt): coverage, flags raised + disposition, suspensions.
- **Two-adult guardian-visibility suspension (C.8):** event-sourced append-only `dm_visibility_suspension` (0032; per-**student** by default, `thread_id` optional). `DmVisibilitySuspensionService.initiate` (`dm.suspend_guardian_visibility`, safety officer) requires a recorded reason, records a 90-day expiry (`DM_VISIBILITY_SUSPENSION_MS`) + surfaces/records the mandatory-reporter checkpoint (returns Ohio hotline content — placeholder), and does **not** take effect until `.acknowledge` (`dm.acknowledge_visibility_suspension`, director / another safety officer). The service enforces the second adult is **not a mentor in the chapter** and **≠ the initiator**. While active (acknowledged, unexpired, non-revoked), `DmThreadService.isPartyToThread` removes the guardian (the other three parties unaffected); **auto-restores** after 90 days (computed at `now`). `.revoke` lifts early. Every step logged.
- **Mentor-departure freeze (C.15):** append-only `dm_thread_freeze` (0032) + `runDmFreezeOnDeparture` (sweep-composable, no HTTP route) — freezes/preserves a departed mentor's threads, recording the **handoff decision**. `DmThreadService.sendMessage` rejects a send to a frozen thread (`DmThreadFrozenError`, even if the pair is re-enabled); reads still work; never deleted.
