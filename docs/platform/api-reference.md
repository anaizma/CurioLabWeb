...
- **Side effect — the opening timeline entry.** In the same transaction as the `application` insert + lead conversion, submit appends a `null → submitted` `application_event` (`from_status = null`, `to_status = 'submitted'`, `actor_id = null` — the applicant is not an authenticated actor, `note = 'Application submitted'`), so `GET /api/ops/applications/{id}` shows a **"Submitted"** history entry immediately rather than an empty timeline.
- **Name derivation (robust to split/combined forms).** The `application.applicant_name` / `guardian_name` are derived from the 2A parent facts, **preferring the definition SPLIT keys** and falling back to the combined keys: student name = `childName` if present, else `join([childFirstName, childLastName])`; guardian name = `guardianName` if present, else `join([guardianFirstName, guardianLastName])`; guardian email = `guardianEmail`. `Stage2ParentFactsIncompleteError` (400) fires only when BOTH the combined and the split forms of a required name (or the email) are absent — so submit works for a director-edited form and for a client that sends only split names.
...
- **Path param:** `id` (application id). **Request body:** `action` (required; one of `screen`, `schedule-interview` (alias `scheduleInterview`), `accept`, `decline`, `withdraw`, `reopen`); `note` (string, optional). For `schedule-interview` only: `interviewAt` (optional — an ISO timestamp for the scheduled slot; validated as a real timestamp, an unparseable value is `400 InvalidInterviewDateError`) and `interviewLocation` (optional string — room / video link / address). Both are optional (a director may flip to `interview_scheduled` and fill the slot later); when supplied they are stored on `application.interview_at` / `application.interview_location` in the SAME transaction as the status flip and the event. A **reschedule** is this same action setting a new time on an already-`interview_scheduled` application (the `interview_scheduled → interview_scheduled` self-edit is not a state change; scope note — the state machine's legality check governs the STATUS edge, not re-setting the slot).
- **Response:** for `reopen` → `201 { applicationId, reopenedFromId }`; for the other actions → `200 { applicationId, from, to }`.
- **Errors:** `400` missing/unknown `action` or `InvalidInterviewDateError` (unparseable `interviewAt`); `403`; `404` `ApplicationNotFoundError`; `409` `IllegalTransitionError`.
...
- **`200`:** `{ applicationId, status, submittedAt (ISO), chapterId, termId: string|null, termName: string|null, student: { fullName: string|null, gradeLevel: string|null, school: string|null, contactEmail: string|null }, guardian: { fullName: string|null, email: string|null }, interview: { at: string|null (ISO), location: string|null }, answers: { stage2a: {…}, stage2b: {…}, stage2c: null }, form: { formId, version: number, definition: {…} } | null, history: [{ from: string|null, to, at (ISO), note: string|null }] }`. **`interview`** is the scheduled slot captured on `schedule-interview` (`application.interview_at` / `interview_location`); both `null` on an application not yet scheduled. `history` includes the opening `{ from: null, to: 'submitted', note: 'Application submitted' }` entry written at 2C submit, oldest first. `student` carries the FULL applicant record (name from `applicant_name`, grade/school from the draft's `parentAnswers.gradeEntering`/`schoolName`, contact = `guardian_email ?? applicant_contact_email`); `termId`/`termName` are derived by date-containment. `stage2a`/`stage2b` are the funnel draft's **complete, unmodified** parent/student answer blobs (every key — the questionnaire the parent reviewed); there is no separate `stage2c` blob (always `null`). **`form` (override 2 — version stamping):** the exact application-form **version the applicant saw**, stamped at submit — `definition` is that version's questions, so the portal renders the answers against them **even after the director later edits and republishes the form**. `null` for a legacy application submitted before form stamping. `history` is the `application_event` trail, oldest first. `404` if unknown; `403` cross-chapter.
...
## 6a-bis. Editable application-form definition (`application-form-definition-spec.md`)

A chapter director edits the questions applicants answer (parent Stage-2A + student Stage-2B); the apply funnel renders from the stored **published** definition, and the director portal renders each application against the version the applicant saw. Backed by the versioned, per-chapter `application_form` table (migration `0034`; `{ id, chapter_id (NULL = platform default), version (monotonic per chapter), status 'draft'|'published', definition jsonb, created_by, created_at, published_at }`, `unique(chapter_id, version)`). **Versions are retained** — a new version is a NEW row; a published row is never mutated (role-level `SELECT`+`INSERT` grant). The `definition` wire shape **matches the frontend model** (`lib/portal/director/application-form.ts`) exactly, so the editor swaps its localStorage load/save for these two calls with **no reshaping**.

**Two program-lead overrides applied** (recorded here + in the spec):
- **Override 1 — student keys stay constrained to the CODE allowlist.** Every `section.id === 'student'` question `key` MUST be in `STAGE2_STUDENT_ALLOWED_FIELDS` (`packages/app/src/config.ts`), else `400`. The allowlist is **not** chapter-scoped/extensible. Directors may reword a student question (label/help/options) but may not add a student key off the allowlist — the COPPA invariant that 2B collects nothing identifying. The existing `saveStudentSection` allowlist+regex guard is unchanged (defence in depth).
- **Override 2 — the submitted form version is stamped.** `startStage2` captures the chapter's currently-**published** form (`form_id` + `form_version`) onto the draft; `submitStage2` carries it to the `application`; `GET /api/ops/applications/{id}` returns that version's `definition` (see `form` above).

### `GET /api/ops/application-form` — read the current form (`application.read`)
- **Query:** `?chapterId=` (admin; a director is scoped to their own chapter).
- **`200`:** `{ chapterId: string|null, version: number, status: "draft"|"published", definition: { version?, sections: [ { id: "parent"|"student", title, description, questions: [ { id, key, label, type, required, help?, options?, fixed? } ] } ] } }`. Returns the chapter's **latest published** form if any, else its **latest draft**, else the **platform default** (`chapterId: null`) when the chapter has none. `403` for a non-director / cross-chapter caller (opaque; `runAuthed`, so no session is also `403`).
- **`type` ∈** `short_text | long_text | email | phone | date | dropdown | multiple_choice | checkboxes | consent`. **`fixed: true`** marks the enrollment-critical parent identity + consent fields (locked `key`/`type`/`required`, not removable — only wording/options editable).

### `PUT /api/ops/application-form` — save a new version (`application.form.manage`)
- **Body:** `{ definition, publish?: boolean }` (admin may target a chapter with `?chapterId=` or a body `chapterId`). **Server-side validation** (all `400 { error: "invalid_request" }` on failure): student keys on the allowlist (override 1); no identifying student keys (`STAGE2_IDENTIFYING_KEY_PATTERN`); every `fixed` field structurally unchanged vs the platform default (`key`/`type`/`required`; only label/help/options may change — removal or restructuring rejected); keys unique within a section, non-empty, slug-safe; choice types (`dropdown`/`multiple_choice`/`checkboxes`) carry ≥1 option.
- **On success:** bumps `version` (monotonic per chapter), inserts a NEW row, `status='published'` + `published_at` when `publish` is true, else `'draft'`. Writes an `audit_entry` (`application.form.saved` / `application.form.published`, `detail { version, publish }`).
- **`200`:** `{ chapterId, version, status, definition }` (the stored row). `403` non-director / cross-chapter; `400` validation failure.
- **Capability:** the new `application.form.manage` (chapter-scoped write, `chapter_director`; `platform_admin` via the override; a read-only `platform_staff` cannot save). Route manifested (`PUT /api/ops/application-form`).

### Public funnel reads carry the published definition (no ops session)
The token-gated Stage-2 responses (`POST /api/public/stage2/start`, `/draft`, `/student-draft`) now include a **`form`** field — `{ formId, chapterId: string|null, version, definition }` — the RESOLVED **published** definition for the applicant's chapter (falls back to the platform default). A draft edit is **never** shown to applicants until published. This lets the apply pages render the questions the parent/student answer without a director session.
